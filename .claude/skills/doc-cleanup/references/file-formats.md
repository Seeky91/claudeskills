...
Files mis à jour : .claude/doccleanup_coverage.md (+1 ligne). Code nettoyé non commité — review via `git diff`.
```
...
Je lance ? (go / ajuster la commande de validation / cibler une zone précise via /doccleanup <path>)
```
...
Files mis à jour : .claude/doccleanup_coverage.md (+<Z-traitées> lignes). Tout est non commité — review via `git diff`, puis commit à ta main.
```
...
```
Aucun fichier source modifié dans l'arbre de travail. Si ton travail de session est déjà commité, cible les chemins via `/doccleanup <path>`.
```
