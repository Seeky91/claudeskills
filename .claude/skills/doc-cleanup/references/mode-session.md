...
Playbook chargé par SKILL.md sur `/doccleanup-session [--touched]`. Nettoie les fichiers modifiés pendant la session courante. Charger `references/doctrine.md` avant d'exécuter. Conventions transverses : cf. SKILL.md.
...
**Cas limites** :
- **Aucun changement** (arbre propre) → `session:none` : *"Aucun fichier modifié dans l'arbre de travail. Si ton travail de session est déjà commité, cible les chemins via `/doccleanup <path>`."* Et terminer.
- **Repo non-git** → pas de signal de session fiable : l'annoncer et suggérer `/doccleanup <path>`.
- Beaucoup de fichiers (≳ 8) ou très gros → **fan-out** sérialisé (cf. `references/orchestration.md`) ; sinon **main-loop**.
...
2. **Ligne de couverture** (delta, en tête de `<PROJECT_ROOT>/.claude/doccleanup_coverage.md`) : mode `session`, scope = `session (<N> files)` (ou `session --touched (<N> files)`). Cf. `references/file-formats.md`.
