...
1quater. **Lentille composition root / niveau d'abstraction (pour `ARC`).** Si la zone contient un landmark architectural (cf. `B.5`), examiner les composition roots que la zone possède : pas seulement l'entrypoint applicatif, aussi les roots locales de sous-systèmes (runtime, worker, web/API, router, pipeline, façade publique structurante). Vérifier si chaque root raconte une suite d'étapes au même niveau d'abstraction, ou si elle mélange orchestration, wiring bas niveau, configuration détaillée, construction de champs internes et logique métier. Produire un finding seulement avec friction concrète (séquence non scannable, parent qui connaît trop les internes du sous-système, changements locaux récurrents dans la root parente). Reco bornée : introduire un constructeur/factory nommé possédé par le sous-système, tout en gardant visibles les politiques globales ; ne pas proposer de wrappers triviaux.
