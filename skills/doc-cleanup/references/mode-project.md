...
Playbook chargé par SKILL.md en mode **project**. Campagne de nettoyage de **tout** le projet, zone par zone, via des sous-agents lorsqu'ils sont disponibles ou via la boucle principale sinon, avec couverture persistée pour la reprise. Charger `references/doctrine.md` et `references/orchestration.md` avant d'exécuter. Conventions transverses (git read-only, validation par zone, dates, delta) : cf. SKILL.md.

## A. Bootstrap

1. Si `<STATE_DIR>` n'existe pas : le créer.
2. Si `<STATE_DIR>/doccleanup_coverage.md` est absent : le créer avec `# Doc-cleanup coverage\n\n`. Résoudre `<STATE_DIR>` depuis le root projet, jamais depuis le `cwd` brut.
...
1. **Lire `<STATE_DIR>/doccleanup_coverage.md`** en entier. Parser les lignes `- YYYY-MM-DD — <zone> — <mode> — …` en couples `(zone, date)`.
2. **`zones_couvertes`** = map `<zone> → date de couverture la plus récente`, sur les lignes de mode `project` ou `zone` (chemins) dont la validation n'est **pas** `tests KO` (une passe KO reste pending, cf. `references/file-formats.md`). Les lignes `session (…)` ne portent pas sur une zone d'inventaire → ignorées pour la couverture (un nettoyage de session antérieur rend juste une zone ultérieure moins chargée).
3. **Staleness** (repo git uniquement) : une zone couverte est **revalidée stale** si du code y a bougé depuis son nettoyage — `git log -1 --format=%cd --date=short -- <zone>` postérieur **ou égal** à sa date de couverture (l'égalité de jour compte comme stale : impossible de savoir si le commit précède ou suit la passe ; mieux vaut un re-balayage pour rien que du code nouveau raté). *Auto-correcteur* : le commit de nettoyage de l'utilisateur peut déclencher un stale **une fois** ; l'agent re-balaie, trouve la zone propre, ré-écrit une ligne `0 supprimés` à jour, et la zone ressort du stale dès le jour suivant. Repo non-git → pas de staleness, couverture par chemin seul.
...
6. Si `zones_pending` est vide (tout couvert **et** rien de stale) : annoncer que tout le projet est à jour, et proposer soit un re-balayage complet (ignorer la couverture), soit d'invoquer le mode zone avec un chemin explicite. Attendre.
...
1. **Choisir l'exécutant** : si les sous-agents sont disponibles et autorisés, en instancier un à contexte vierge avec le briefing de `references/orchestration.md > Briefing d'un sous-agent de zone`. Sinon, traiter la zone en main-loop segmentée avec ce briefing comme checklist. Une petite zone unique ≲ 1500 LoC reste en main-loop.
2. **Produire le résumé** (fichiers inspectés, supprimés, renames + sites + outil, docs dé-driftées, fichiers modifiés, non-traités, incertitudes). Ne **pas** charger le code de plusieurs zones simultanément dans le contexte principal.
...
5. **Écrire la ligne de couverture** (delta, en tête de `<STATE_DIR>/doccleanup_coverage.md`) : cf. `references/file-formats.md`.
